...
const Header = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex flex-col md:flex-row md:items-center md:px-36 px-10 text-white bg-gray-900">
      <a
        href="/"
        className="inline-block p-4 text-xl text-green-400 font-audiowide italic capitalize font-semibold"
      >
        Shandy
      </a>
      {showMenu ? (
        <IoCloseSharp
          ...
          className="md:hidden text-4xl absolute right-8 top-4"
        />
      ) : (
        <CgMenuHotdog
          onClick={() => setShowMenu(!showMenu)}
